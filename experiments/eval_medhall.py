"""Zero-shot evaluation of a FEVER-trained ClauseGuard on MedHallBench.

Demonstrates that the FEVER-trained verifier generalises to medical
LLM hallucinations, the headline EU AI Act medical-deployment case
for the paper.
"""
...
log = logging.getLogger("eval_medhall")


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate ClauseGuard on MedHallBench.")
    ...
    logger = ExperimentLogger(
        ...
        run_meta={"experiment": "eval_medhall", "seed": args.seed},
    )

    rows = load_medhall(max_samples=args.max_samples)
    ...
    bin_mask = labels < 2
    if sums.ndim == 2:
        refute_score = sums[bin_mask, 1] - sums[bin_mask, 0]
    else:
        refute_score = (preds[bin_mask] == 1).astype(float)
    au = auroc((labels[bin_mask] == 1).astype(int), refute_score)
    for row, p, sc in zip(rows, preds, sums):
        logger.log_prediction(
            claim_id=row["claim_id"],
            true_label=int(row["label"]),
            pred_label=int(p),
            class_scores=list(map(float, sc)) if hasattr(sc, "ndim") and sc.ndim else [float(sc)],
        )
    summary = {
        "experiment": "eval_medhall",
        "seed": args.seed,
        "n_samples": len(rows),
        "accuracy": float(acc),
        "macro_f1": float(f1),
        "binary_auroc_supported_vs_hallucinated": float(au),
